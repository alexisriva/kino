...
import { X, Sparkles, Send, Film, MessageSquare, User, CheckCircle2 } from 'lucide-react';
...
export function RequestMediaModal({ onClose, onRequestSubmitted }: RequestMediaModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (res.success) {
      setSuccessMessage(res.message || 'Your watch request was submitted successfully!');
      ...
      <div className="relative w-full max-w-xl my-8 p-6 sm:p-8 rounded-lg bg-[#1f2022] border border-[#292a2c] text-[#e3e2e5] shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        {successMessage ? (
          <div className="py-12 flex flex-col items-center justify-center text-center space-y-3">
            <CheckCircle2 className="w-12 h-12 text-[#f2ca50] animate-bounce" />
            <h3 className="text-lg font-bold font-headline text-[#e3e2e5]">Request Received!</h3>
            <p className="text-xs text-[#99907c] max-w-sm">{successMessage}</p>
          </div>
        ) : (
          <>
            ...
            <div className="p-4 rounded-md bg-[#121315] border border-[#4d4635] mb-6 flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-[11px] text-[#99907c] mt-0.5">
                  Search any movie, show, or documentary to auto-populate poster and details.
                </p>
              </div>
              <button
                type="button"
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="p-4 rounded-md bg-[#121315] border border-[#292a2c] space-y-3">
                <h4 className="text-xs font-bold text-[#e3e2e5] font-headline">Requester Info (Optional)</h4>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  <div>
                    <label className="flex items-center gap-1 text-[11px] font-semibold text-[#99907c] mb-1">
                      <User className="w-3 h-3 text-[#f2ca50]" /> Your Name / Nickname
                    </label>
                    <input
                      type="text"
                      ...
                      className="w-full px-3 py-1.5 rounded-md bg-[#1b1c1e] border border-[#292a2c] text-xs text-[#e3e2e5] focus:outline-none focus:border-[#f2ca50]"
                    />
                  </div>
                  <div>
                    <label className="flex items-center gap-1 text-[11px] font-semibold text-[#99907c] mb-1">
                      <MessageSquare className="w-3 h-3 text-[#f2ca50]" /> Why should I watch this? (Note)
                    </label>
                    <input
                      type="text"
                      placeholder="e.g. Great plot twist and acting!"
                      value={requesterNote}
              ...
              <div className="pt-4 flex items-center justify-end gap-3 border-t border-[#292a2c]">
                ...
                <button
                  type="submit"
                  disabled={submitting}
                  className="flex items-center gap-1.5 px-6 py-2 rounded-md bg-[#f2ca50] text-[#121315] font-headline font-bold text-xs shadow-md cursor-pointer"
                >
